...
std::vector<std::string> StringHelper::split(const std::string& src)
{
  ...
  do
  {
    while ((isspace(src[index], loc) || src[index] == ','|| src[index] == ':') && src[index] != '\n')
    {
      ++index;
      if (temp.size() > 0)
      {
...
int main()
{
  ...
  std::string conv1 = Convert<double>::toString(3.1415927);
  std::string conv2 = Convert<int>::toString(73);
  std::string conv3 = Convert<std::string>::toString("a_test_string plus more");
  ...
  std::cout << Convert<double>::fromString(conv1) << ", ";
  std::cout << Convert<int>::fromString(conv2) << ", ";
  std::cout << Convert<std::string>::fromString(conv3);
