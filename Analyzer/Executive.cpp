...
void CodeAnalysisExecutive::processSourceCode(bool showProc) {
	...
	for (auto file : csharpFiles()) {
		...
		if (!Rslt::running() || (!Demo::running() && !Rslt::running()))  // parse file
			Dbug::write("\n\n  opening file \"" + pRepo_->package() + "\"");
...
void _showVert(Vertex<std::string, std::string>& v)
{
	std::cout << "\n  " << v.id();
}

template<typename V, typename E>
void _TshowVert(Vertex<V, E>& v)
{
	std::cout << "\n  " << v.id();
}

graph AddtoGraph(NoSQLDB::NoSqlDb<std::string> db)
{
	graph g;
	try
	{
		...
		std::cout << "\n -----------------------------";
		NoSQLDB::Keys ikeys = db.keys();

		int count = 1;

		for (NoSQLDB::Key key : ikeys)
		{
			vertex vBase(key);
			g.addVertex(vBase);

			std::vector<std::string> temp = db.value(key).children.getValue();
			for (unsigned i = 0; i < temp.size(); i++)
			{
				vertex vChild(temp[i]);
				g.addVertex(vChild);
				Convert<int> cc;
...
void buildNoParentList(NoSQLDB::NoSqlDb<std::string> db)
{
	NoSQLDB::Keys ikeys = db.keys();
	std::vector<std::string> noParent;
	for (NoSQLDB::Key key : ikeys) {
		if (db.value(key).children.getValue().size() == 0)
			noParent.push_back(key);
	}
	if (noParent.size() != 0)
	{
		std::ofstream str;
		if (!FileSystem::Directory::exists("../ServerRepository/NoParent"))
			FileSystem::Directory::create("../ServerRepository/NoParent");
		str.open("../ServerRepository/NoParent/np.txt");
		for (std::string np : noParent)
			str << np << ":";
		str.close();
	}
}
...
void testDepAnal(TypeTable<StrData> tt, std::vector<std::string> files, std::string path)
{
	...
	for (unsigned i = 0; i < files.size(); i++) {
		DepAnal da(files[i], tt);
		da.saveToken();
		NoSQLDB::Element<std::string> tempElem = da.DepAnalyzer();
		//if (tempElem.children.getValue().size() != 0)
		{
			Convert<int> cc;
			tempElem.data = cc.toString(count);
			da.savetoDb(tempElem, db);
			count++;
		}
		publisher pu;
		pu.fileToHtml(files[i], tempElem);
		std::cout << "  - Req #3";
	}
	NoSQLDB::Keys ikeys = db.keys();
	for (NoSQLDB::Key key : ikeys) {
	...
	buildNoParentList(db);
	publisher pu;
	pu.CreateIndex(db);
	std::cout << "\n";
	std::cout << "  Open Index Webpages" << "\n";
	std::string file_path = "..\\ServerRepository\\HTML\\Index.htm";
	std::string command("start \"\" \"" + file_path + "\"");
	std::system(command.c_str());
	graph g = AddtoGraph(db);
	std::cout << "\n  Strong Components Analyze";
	std::cout << "\n ---------------------------\n";
	std::vector< std::vector<std::string> > vvs = g.Scc();
	std::string str = GraphToXmlString(g);
	std::cout << str << "\n";
	std::ofstream xmlout("Dependency.xml");
	xmlout << str << "\n";
	std::string sscstr = GraphToXmlString(vvs);
	std::cout << sscstr << "\n";
	std::ofstream ssout("StrongComponent.xml");
	ssout << sscstr << "\n";
	std::cout << "\n";
}
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		std::ostringstream tOut("CodeAnalysis - Built in VS 2015 with C++   - Req #1");
		...
		IFileMgr* pFmgr = FileMgrFactory::create(argv[1]);
		FileHandler fh;
		DirHandler dh;
		DoneHandler dnh;
