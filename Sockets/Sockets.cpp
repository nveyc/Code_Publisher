...
SocketSystem::SocketSystem()
{
  ...
  if (iResult != 0) {
    Show::write("\n  WSAStartup failed with error = " + Convert<int>::toString(iResult));
  }
}
...
{
  int bytesRecvd = 0, bytesLeft = bytes;
  byte* pBuf = buffer;
  while (bytesLeft > 0)
  {
...
bool Socket::waitForData(int timeToWait, int timeToCheck)
{
  int MaxCount = timeToWait / timeToCheck;
  static int count = 0;
...
bool SocketConnecter::connect(const std::string& ip, size_t port)
{
  ...
  std::string sPort = Convert<size_t>::toString(uport);
  // Resolve the server address and port
  const char* pTemp = ip.c_str();
  iResult = getaddrinfo(pTemp, sPort.c_str(), &hints, &result);  // was DEFAULT_PORT
  if (iResult != 0) {
    Show::write("\n\n  -- getaddrinfo failed with error: " + Convert<int>::toString(iResult));
    return false;
  }
  for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {   // Attempt to connect to an address until one succeeds
    ...
    if (socket_ == INVALID_SOCKET) {
      ...
      Show::write("\n\n  -- socket failed with error: " + Convert<int>::toString(error));
      return false;
    }
    ...
    if (iResult == SOCKET_ERROR) {
      ...
      int error = WSAGetLastError();
      Show::write("\n  WSAGetLastError returned " + Convert<int>::toString(error));
      continue;
    }
    break;
  }
  ...
  if (socket_ == INVALID_SOCKET) {
    int error = WSAGetLastError();
    Show::write("\n\n  -- unable to connect to server, error = " + Convert<int>::toString(error));
    return false;
  }
  return true;
}
...
bool SocketListener::bind()
{
  ...
  for (auto pResult = result; pResult != NULL; pResult = pResult->ai_next)
  {
    ...
    if (iResult == SOCKET_ERROR) {
      int error = WSAGetLastError();
      Show::write("\n\n  -- bind failed with error: " + Convert<int>::toString(error));
      socket_ = INVALID_SOCKET;
      continue;
    }
    else
...
bool SocketListener::listen()
{
  ...
  if (iResult == SOCKET_ERROR) {
    int error = WSAGetLastError();
    Show::write("\n\n  -- listen failed with error: " + Convert<int>::toString(error));
    socket_ = INVALID_SOCKET;
    return false;
  }
...
Socket SocketListener::accept()
{
  ...
  if (!clientSocket.validState()) {
    acceptFailed_ = true;
    int error = WSAGetLastError();
    Show::write("\n\n  -- server accept failed with error: " + Convert<int>::toString(error));
    Show::write(
